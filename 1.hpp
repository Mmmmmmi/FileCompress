...
/*
		...
		1. 首先 因为hash函数的原因，因此 需要先将两个字符存入hash表中
			hash函数是将 之前的hash地址拿过来 进行左移 将之前三个字符的第一个字符有关的数据 左移出去
			然后将本次的字符加进去，并且 一次得到的是和三个字符都有关的，因此先计算两次hash地址

		2. 开始读取字符
			从窗口中遇到一个字符， 根据hash函数 在hash表中查找
			如果对应位置的下标为0 那么说明是第一次存进hash表中
			如果对应位置处的下标不为0 说明之前出现过 因此返回下标

		3. 得到最长匹配长度之后，将其存入文件
			在这里 准备用 两个字节 USH 来表示距离对
						用一个字节  UCH 来表示匹配长度
		4. 先写小文件的  大文件的之后完善
...
public:
	...
	void WriteFlag(FILE* fOutFlag, UCH& flag, char& fileLeftCOunt, int type)
	{
		//fOutFlag  文件指针
		//flag		标记值
	...
	void Compress(const std::string& fileName)
	{
		...
		USH matchHead = 0;		//当字符组插入后，返回的数据  如果之前插入过 那么 返回上一个的下标 如果没有 返回0
								//也就是返回匹配链的链首位置
	...
	void GetMaxMatch(USH & machtListHead, USH & maxDistant, UCH & maxMatch)
	{
		...
		while (nextMatch) {		//当没有走到list的最后一个的时候 就一直走
			...
			while (_buffer[curMatchPos++] == _buffer[curStart++]) {		//如果匹配字符相同 那么本次匹配长度加一
	...
	void UnCompress(const std::string & fileName)
	{
		...
		ULL flagSize = 0;
		fseek(fInFlagSize, 0 - sizeof(flagSize), SEEK_END);
...
private:
	...
	UCH* _buffer;					//滑动窗口
	USH _lookAhead;					//表示剩余多少元素待压缩
	USH _start;						//表示当前压缩到窗口的哪个下标
};